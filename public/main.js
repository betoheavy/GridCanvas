...
function main() {
    ...
    IL.onLoad(imageArray =>{
        ...
        for(let image of imageArray){
            SpriteArray.push(new Sprite(image));
        }
        ...
        SpriteArray[1].collide = true;
        SpriteArray[2].collide = true;

        let customGrid = [];
        for (let x = 0; x < 18; x++){
            let col = [];
            for (let y = 0; y < 18; y++){
                if (y == 0 || y == 17 || x == 0 || x== 17) col.push(SpriteArray[1]);
                else col.push(SpriteArray[0]);
            }
            customGrid.push(col);
        }

        //dibuja un pasillo en la esquina para ver si Floppa pasa
        customGrid[2][2] = SpriteArray[1];
        customGrid[2][3] = SpriteArray[1];
        customGrid[3][2] = SpriteArray[1];

        background.grid = customGrid;
        player.grid     = [[SpriteArray[2]]];
